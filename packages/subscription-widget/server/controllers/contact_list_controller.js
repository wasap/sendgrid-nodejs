...
function addUserToList(emailBody, callback) {
  ...
  var ignoreFields = ['ip', 'sg_event_id', 'sg_message_id', 'useragent', 'event', 'url_offset', 'time_sent', 'timestamp', 'url', 'type', 'smtp-id'];
  var customFields = [{}];
  var customFieldArr = [];
  ...
  checkAndAddCustomFields(customFieldArr, function () {
    ...
    const timeElapsed = (Date.now() - timestamp) / 1000; // Confirm email type is opt in and link has been clicked within 1 day
