...
class Parse {
  ...
  keyValues() {
    return this.keys.filter(key => this.payload[key]).map(key => ({
  ...
  _getAttachments(callback) {
    return callback(this.files.filter(file => fs.existsSync(file.path)).map((exists, idx) => [exists, this.files[idx]]).filter(([exists, _]) => exists).map(([_, file]) => {
