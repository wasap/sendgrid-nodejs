...
class ContactImporter extends EventEmitter {
  constructor(sg, options = {}) {
    super();
    this.sg = sg;
    this.pendingItems = []; // Number of items to send per batch.

    this.batchSize = options.batchSize || 1500; // Max number of requests per rate limit period.

    this.rateLimitLimit = options.rateLimitLimit || 3; // Length of rate limit period (miliseconds).

    this.rateLimitPeriod = options.rateLimitPeriod || 2000; // Create a throttler that will process no more than `rateLimitLimit` requests every `rateLimitPeriod` ms.
  ...
  push(data = []) {
    data = Array.isArray(data) ? data : [data]; // Add the new items onto the pending items.

    const itemsToProcess = this.pendingItems.concat(data); // Chunk the pending items into batches and add onto the queue
  ...
  _sendBatch(context, data, callback) {
    ...
    request.body = data;
    context.sg.API(request).then(response => {
  ...
  */
  ...
  _setupQueue() {
    ...
    this.queue = queue(ensureAsync(this._worker)); // When the last batch is removed from the queue, add any incomplete batches.
