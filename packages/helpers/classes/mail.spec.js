...
describe('Mail', function () {
  describe('#527', function () {
    it('shouldn\'t convert the headers to camel/snake case', function () {
      ...
      expect(mail.headers['List-Unsubscribe']).to.equal('<mailto:test@test.com>');
      expect(mail.toJSON().headers['List-Unsubscribe']).to.equal('<mailto:test@test.com>');
    });
  });
  describe('#689', function () {
    ...
    it('should ignore substitutions if templateId is dynamic', function () {
      ...
      expect(mail.dynamicTemplateData).to.deep.equal({
        test1: 'Test 1',
        test2: 'Test 2'
      });
      expect(mail.personalizations[0].dynamicTemplateData).to.deep.equal({
        test1: 'Test 1',
        test2: 'Testy 2',
        test3: 'Testy 3'
      });
